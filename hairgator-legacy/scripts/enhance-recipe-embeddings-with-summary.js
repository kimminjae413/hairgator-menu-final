// scripts/enhance-recipe-embeddings-with-summary.js
// 요약문이 있는 레시피의 recipe_embedding을 "요약문 + 원본 텍스트"로 재생성
...
// 메인 실행
async function enhanceEmbeddings() {
  try {
    console.log('🚀 요약문 포함 임베딩 재생성 시작\n');
    ...
    while (hasMore) {
      ...
      url.searchParams.set('select', 'id,sample_code,recipe_summary_ko,recipe_full_text_ko');
    ...
    for (let i = 0; i < recipes.length; i++) {
      ...
      try {
        ...
        if (!recipe.recipe_full_text_ko || recipe.recipe_full_text_ko.trim().length === 0) {
          console.log(`⚠️  [${i+1}/${recipes.length}] ${recipe.sample_code}: 원본 텍스트 비어있음, 건너뜀`);
          skipCount++;
          continue;
        }

        // 요약문 + 원본 텍스트 결합
        const combinedText = `${recipe.recipe_summary_ko}\n\n${recipe.recipe_full_text_ko}`;

        // 임베딩 생성
        const embedding = await generateEmbedding(combinedText);

        // DB 업데이트 (recipe_embedding 덮어쓰기)
        await supabaseRequest('PATCH', `recipe_samples?id=eq.${recipe.id}`, {
          recipe_embedding: embedding
        });

        console.log(`✅ [${i+1}/${recipes.length}] ${recipe.sample_code}: 향상된 임베딩 생성 완료`);
        successCount++;
