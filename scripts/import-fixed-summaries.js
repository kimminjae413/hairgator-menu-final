// scripts/import-fixed-summaries.js
// 수정된 TSV 파일 (v3-fixed)에서 레시피 요약문을 읽어서 DB에 업데이트
...
async function importSummaries() {
  try {
    console.log('📚 수정된 레시피 요약문 임포트 시작\n');
    console.log('='.repeat(60));

    // 수정된 TSV 파일 읽기
    const tsvPath = path.join(__dirname, '..', 'data', 'recipe-summaries-female-cut-v3-fixed.tsv');
    ...
    for (const summary of summaries) {
      try {
        ...
        console.log(`✅ ${summary.sample_code}: ${recipes.length}개 레시피에 요약문 업데이트`);
