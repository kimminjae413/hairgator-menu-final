-- ============================================================================
-- Supabase 임베딩 컬럼 타입 수정 스크립트 v2 (오류 수정)
...
WHERE recipe_embedding_backup IS NOT NULL
  AND LENGTH(recipe_embedding_backup) > 10;
...
WHERE summary_embedding_backup IS NOT NULL
  AND LENGTH(summary_embedding_backup) > 10;
...
WHERE embedding_backup IS NOT NULL
  AND LENGTH(embedding_backup) > 10;
...
SELECT
  ...
  COUNT(image_embedding_backup) as image_backup_count,
  COUNT(recipe_embedding_backup) as recipe_backup_count,
  COUNT(summary_embedding_backup) as summary_backup_count
FROM recipe_samples;
...
SELECT
  ...
  COUNT(image_embedding) as image_emb_count,
  COUNT(embedding_backup) as embedding_backup_count,
  COUNT(image_embedding_backup) as image_backup_count
FROM theory_chunks;
...
-- 2. 각 테이블의 임베딩 카운트가 백업 카운트와 일치하는지 확인하세요
-- 3. node scripts/verify-embedding-fix.js 실행
-- 4. 챗봇 검색 기능을 테스트하세요
-- 5. 모든 것이 정상이면 백업 컬럼을 삭제하세요 (선택사항)
--
-- ============================================================================
