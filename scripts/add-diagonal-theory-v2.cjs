...
async function checkExistingStructure() {
  ...
  const { data } = await supabase
    .from('theory_chunks')
    .select('importance_level, chunk_type, sub_category, category_code')
    .limit(10);

  console.log('importance_level 샘플:', [...new Set(data?.map(d => d.importance_level))]);
  console.log('chunk_type 샘플:', [...new Set(data?.map(d => d.chunk_type))]);
  console.log('sub_category 샘플:', [...new Set(data?.map(d => d.sub_category))]);
  console.log('category_code 샘플:', [...new Set(data?.map(d => d.category_code))]);

  return data;
}

// 이미지 7장에서 추출한 이론 내용 (수정된 버전)
...
async function addDiagonalTheory() {
  ...
  for (let i = 0; i < diagonalTheoryChunks.length; i++) {
    ...
    try {
      ...
      // 2. Supabase에 삽입 (기존 형식에 맞춤 - 불필요한 필드 제거)
      const insertData = {
        ...
        // importance_level, chunk_type 등 제약조건 있는 필드 제외
        is_verified: true,
        quality_score: 1.0,
