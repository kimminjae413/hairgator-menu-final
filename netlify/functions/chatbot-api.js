...
참고 레시피의 A존, B존 내용을 여기에 작성. 한 문장당 하나의 번호를 붙인다.

1. B존은 후대각 섹션(DBS, 뒤쪽으로 기울어진 대각선 슬라이스)과 천체축 각도 75도(두피에서 약 75도 들어올림), 다이렉션 D2~반대쪽 D6 기점까지 이동 디자인라인(점점 길이가 변하도록 자름)을 활용하여 그래쥬에이션(아래가 짧고 위가 긴 층)으로 진행한다.

---

**[Internal] (Over Zone)**

참고 레시피의 C존, 프린지 내용을 여기에 작성. 번호는 1부터 다시 시작.

1. C존은 후대각 섹션(DBS)과 천체축 각도 75도, 디스커넥션(층 분리), 변이분배, 다이렉션 D4~D8 기점까지 이동 디자인라인을 활용하여 진행한다.

2. C존은 파이섹션(부채꼴 모양으로 모아 잡는 방식)과 천체축 각도 185도(두피와 거의 평행하게 눕힘), 다이렉션 D8~D4 기점까지 이동 디자인라인으로 진행하고, 사이드 부분은 다이렉션 D4 기점을 축으로 고정 디자인라인(같은 길이 유지)으로 진행한다.

3. 프린지(앞머리) 부분은 가로섹션(HS, 수평 슬라이스)과 천체축 각도 45도(두피에서 45도 들어올림), 다이렉션 D0 기점(정면 중앙)을 활용하여 고정 디자인라인으로 진행한다.

---

### ⚠️ 필수 규칙:
1. 반드시 **[External] (Under Zone)**과 **[Internal] (Over Zone)**으로 구분
2. 각 문장에 번호 매기기: 1, 2, 3... (External과 Internal 각각 1부터 시작)
3. 한 문장 = 한 번호 (참고 레시피의 마침표 기준으로 분리)
4. 모든 전문용어 뒤에 괄호()로 쉬운 설명 추가
5. 존 이름(A존, B존, C존)은 그대로 유지하되, 섹션 구분만 [External]/[Internal]로

### 🚫 절대 금지:
- 참고 레시피 그대로 복사 금지! 괄호 설명을 추가하여 변환
- 마크다운 기호(*, **, #) 사용 금지! 번호(1, 2, 3)만 사용
- 이모지(📐, 💡) 사용 금지
- 중복 문장 금지`
            }]
          }],
