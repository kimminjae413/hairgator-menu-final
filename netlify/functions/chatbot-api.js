...
5. ❌ 스타일 코드 절대 노출 금지: FAL0001, FALP3003, SF1001, FBL2001 등 코드명을 유저에게 보여주지 마세요
6. 대신 "가슴 하단 길이의 S컬 펌", "사이드 프린지 남자 커트" 등 자연어로 표현
`;
...
2. 각 문장에 번호: 1. 2. 3. (각 섹션마다 1부터 시작)
3. 한 문장 = 한 번호 (마침표 기준으로 분리)
...
- 이모지 사용 금지 (📐, 💡, ⭐, ✂️ 등)
- 중복 문장 금지
- 참고 레시피 그대로 복사 금지 (괄호 설명 추가하여 변환)

=== 최종 확인 ===
출력에 별표(*), 해시(#), 이모지가 하나라도 있으면 실패입니다.
오직 일반 텍스트, 숫자 번호(1. 2. 3.), 괄호 설명만 사용하세요.`
            }]
          }],
          tools: [{
            fileSearch: {
