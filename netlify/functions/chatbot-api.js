...
**5. 존 설명:**
- "엑스터널" → "엑스터널 (바깥쪽 머리 - 눈에 보이는 아웃라인)"
- "인터널" → "인터널 (안쪽 머리 - 볼륨과 형태를 만드는 층)"
...
참고 레시피의 존별 구조를 따르되, 다음과 같이 **모든 수치에 괄호 설명 추가**:

**좋은 예시:**
"리셋션 섹션 (측두부 - 구레나룻 위 영역)을 분리한다. 엑스터널 (바깥쪽 아웃라인)은 천체축각도 0도 (머리를 들지 않고 자연스럽게 떨어뜨린 상태)로 아웃라인을 정리한다. 인터널 (안쪽 볼륨층)은 리프트 L8 기점 (180도 - 머리를 완전히 세운 상태), 스퀘어라인 (직각 커팅), 고정 디자인라인 (같은 길이 유지)으로 진행한다."

**나쁜 예시 (이렇게 쓰면 안 됨!):**
"리셋션 섹션을 분리한다. 엑스터널부분은 천체축각도0도로 아웃라인을 정리한다."
→ 설명 없이 용어만 나열하면 안 됩니다!

### 반드시 설명이 필요한 용어들:
1. **각도**: 0도, 67.5도, 75도, 90도, 135도, 180도 → 각도가 무엇을 의미하는지
2. **디렉션**: D0, D4, D6, D8 → 머리의 어느 방향인지
3. **섹션**: 전대각, 후대각, 파이섹션, 수평 → 머리카락을 어떻게 나누는지
4. **기법**: 고정/이동 디자인라인, 디스커넥션, 스퀘어라인 → 어떤 효과가 나는지
5. **존**: 엑스터널, 인터널, 리셋션, C존, F.S.P, S.P → 머리의 어느 부위인지

### 출력 형식:
존별로 구분하여 작성 (A존, B존, C존 또는 리셋션/사이드/백 등)
각 존마다 섹션→리프팅→디렉션→기법 순서로 설명
**모든 전문용어 뒤에 괄호()로 쉬운 설명 필수!**`
            }]
          }],
          tools: [{
            fileSearch: {
